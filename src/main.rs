                        ...
                        b"dem:TINRelief" => {
                            if let Some(mut co) = current_cityobject {
                                co.geometry = vec![CityObjectGeometry {
                                    ...
                                    boundaries: triangles.into_iter().map(|t| Boundary(vec![t])).collect()
                                }];
                ...
                Ok(Event::Text(e)) => {
                    if collect {
                        ...
                        let triangle = TriangleIndices::from_vec(
                            my_vertices.into_iter().take(3).map(|vertex| { let (index, _) = model.vertices.insert_full(vertex); index }).collect()
                        );
...
fn main() {
    ...
    info!("Loaded CityModel with {} objects and {} vertices", model.objects.len(), model.vertices.len());
